...
Describe "Get-PSVault" -Tags Build , Unit{
  ...
  context '
    ...
    Get-PSVault -path "$($temporaryFolder.Fullname)\Pester"
  ' {
    Mock Read-Host {return $SecureString}
    it 'opens the specified vault.' {
      ...
      $script:KeyLocation = $vault.GetSavedKeyLocation()
      {
        Get-PSVault -path "$($temporaryFolder.Fullname)\Pester" -VaultPassword $SecureString
      } | should not throw
    }
    it 'By asking for a password.' {
      Assert-MockCalled -CommandName Read-Host -Times 1
